      ...
      <div className="panel">
        <div className="flex space-between" style={{ marginBottom: 6 }}>
          ...
          <div className="flex" style={{ gap: 8 }}>
            ...
            <span className="badge">{result.source === 'mock' ? 'Mock (no API key)' : 'LLM + recall'}</span>
          </div>
        </div>
        ...
        {result.code_snippet && (
          <div style={{ marginTop: 12 }}>
            <div className="flex space-between" style={{ marginBottom: 6 }}>
              ...
              <button className="button inline" type="button" onClick={() => onCopy(result.code_snippet)}>Copy</button>
            </div>
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const fallback = {
        ...
        root_cause: err.response?.data?.message || 'Backend not reachable',
        fix: 'Ensure backend is running and VITE_API_URL points to it.',
        confidence: 0.1,
        pr_snippet: 'Restore connectivity to the debug service; then rerun analysis.',
        code_snippet: '',
        similar: []
      };
      setResult(fallback);
